...
describe('CrystalTally', () => {
  it('renders the weekly readout as a bare count (no /14 cap suffix)', () => {
    render(<CrystalTally weeklyCount={5} dailyCount={0} monthlyCount={0} />);
    const weekly = screen.getByLabelText(/weekly boss selections/i);
    expect(weekly.textContent).toBe('5');
    expect(weekly.querySelector('.crystal-tally__cap')).toBeNull();
  });
  ...
  it('shows a bare "0" on every plate when all counts are zero', () => {
    render(<CrystalTally weeklyCount={0} dailyCount={0} monthlyCount={0} />);
    expect(screen.getByLabelText(/weekly boss selections/i).textContent).toBe('0');
  ...
  it('renders the raw weekly count even when it exceeds the weekly cap', () => {
    render(<CrystalTally weeklyCount={17} dailyCount={40} monthlyCount={2} />);
    expect(screen.getByLabelText(/weekly boss selections/i).textContent).toBe('17');
    expect(screen.getByLabelText(/daily boss selections/i).textContent).toBe('40');
  });

  it('is a read-only counts display — no Mark Toggle buttons on any plate', () => {
