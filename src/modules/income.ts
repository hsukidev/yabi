...
export function getMuleIncome(selectedBosses: string[], abbreviated: boolean = true): IncomeResult {
  const raw = calculatePotentialIncome(selectedBosses)
  const formatted = formatMeso(raw, abbreviated)
  return { raw, formatted }
}

export function getTotalIncome(mules: { selectedBosses: string[] }[], abbreviated: boolean = true): IncomeResult {
