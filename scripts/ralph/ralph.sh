...
# model: model ID (e.g. ollama-cloud/glm-5.1 for opencode, smart/rush for amp)
...
if [ ! -d "$PROJECT_DIR/scripts/ralph" ]; then
	echo "Error: scripts/ralph/ directory not found"
	exit 1
fi
...
if [ "$CLI_TOOL" = "opencode" ]; then
	echo "Using CLI: $CLI_TOOL (model: ${MODEL:-ollama-cloud/glm-5.1})"
	echo "Share session: $SHARE"
else
	if [ -n "$MODEL" ]; then
		echo "Using CLI: $CLI_TOOL (mode: $MODEL)"
	else
		echo "Using CLI: $CLI_TOOL (default mode)"
	fi
fi
...
for i in $(seq 1 $MAX_ITERATIONS); do
	...
	if [ "$CLI_TOOL" = "opencode" ]; then
		OPENCODE_MODEL=${MODEL:-ollama-cloud/glm-5.1}
		if [ "$SHARE" = "true" ]; then
